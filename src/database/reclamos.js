...
export default class Reclamos {
    ...
    //         const sql = `
    //             UPDATE reclamos 
    //             SET idReclamoEstado = (SELECT idReclamoEstado FROM reclamosestado WHERE descripcion = 'cancelado'), 
    //                 fechaCancelado = NOW() 
    //             WHERE idReclamo = ? AND idUsuarioCreador = ? AND idReclamoEstado = (SELECT idReclamoEstado FROM reclamosestado WHERE descripcion = 'creado')
    //         `;
    //         const [result] = await conexion.query(sql, [idReclamo, idUsuarioCreador]);
